...
export function deriveRecommendationStatus(params: {
  ...
  if (params.hasHistoricalRecommendation) {
    return {
      ...
      title: "The latest intake changed. Re-run downstream stages before relying on the previous recommendation.",
...
export function decisionStatusToneClassName(tone: "fresh" | "warning" | "pending") {
  switch (tone) {
    case "fresh":
