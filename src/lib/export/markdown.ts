...
export function renderDecisionMemoMarkdown(
	memo: Omit<DecisionMemo, "markdown">,
) {
	const sections = [
		...
		...memo.mitigationChecklist.map(
			(mitigation) => `- ${mitigation.checklistItem}`,
		),
		`## Reversible Next Step\n${memo.reversibleNextStep}`,
		"## Kill Criteria",
		...memo.killCriteria.map(
			(criterion) => `- ${criterion.statement} (${criterion.windowText})`,
		),
		"## Evidence Needed",
...
export function exportFilename(slug: string, ext: "md" | "json" = "md") {
	const safe = slug.replace(/[^a-z0-9-]/gi, "-");
	return `${safe}-decision-memo.${ext}`;
}
