...
describe("decision memo export route", () => {
  ...
  test("returns 404 when the memo is missing", async () => {
    getDecisionByIdMock.mockResolvedValue({
      decision: {
        id: "decision_1",
        slug: "take-the-role-123abc",
      },
      memo: null,
    });

    const { GET } = await import("@/app/api/decisions/[decisionId]/export/route");
    const response = await GET(new Request("http://127.0.0.1:3000/api/decisions/decision_1/export"), {
  ...
  test("returns markdown with download and cache-safe headers when the memo exists", async () => {
    ...
    expect(response.headers.get("Content-Disposition")).toContain("attachment; filename=\"take-the-role-123abc-decision-memo.md\"");
